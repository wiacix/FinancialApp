...
export const prepareDataBase = () => {
  db.execSync(`
    ...
    CREATE TABLE IF NOT EXISTS settings (lang varchar(3) NULL, idGlobal int(11) NULL, sumaIconId int(11) NULL, sumaColor varchar(50) NULL, defaultDateType INT NULL, defaultFromDate varchar(50) NULL, defaultToDate varchar(50) NULL, sideMenuName int NULL, pin varchar(10) NULL, touchId int NULL, lastTransfer int NULL, lastDateType int NULL, lastFromDate varchar(50) NULL, lastToDate varchar(50) NULL, lastAccountCode int NULL, defaultTransfer tinyint NULL);
...
export const insertUser = (idGlobal, login, password, name, surname, groupsId, saveLogin) => {
  ...
  db.runSync(
    "UPDATE settings SET idGlobal = ?", idGlobal
  )
  db.runSync(
...
export const updateSettings = (idGlobal, defaultDataType, defaultFromDate, defaultToDate, sideMenuName, sumaColor, sumaIconId, lang, pin, touchId, groupId, tithePercent, defaultTransfer) => {
  db.runSync(
    `UPDATE settings SET defaultDateType = ${defaultDataType}, defaultFromDate = '${defaultFromDate}', defaultToDate = '${defaultToDate}', sideMenuName = ${sideMenuName}, sumaColor = '${sumaColor}', sumaIconId = ${sumaIconId}, lang = '${lang}', pin = '${pin}', touchId = ${touchId}, defaultTransfer=${defaultTransfer} WHERE idGlobal = ${idGlobal}`
  )
  if(groupId!=null) 
    db.runSync(
...
export const EditTransfer = (data, dataFromAcc, dataToAcc) => {
  ...
  db.runSync(
    `INSERT INTO account (Id, Code, Active, Name, Balance, IconId, Color, Status, GroupsId, UpdateDate) VALUES (${dataFromAcc[0]}, ${data.fromAcc}, 1, (SELECT a.name FROM account a WHERE Code=${data.fromAcc} ORDER BY Id desc limit 1), ${dataFromAcc[2]}, (SELECT a2.IconId FROM account a2 WHERE Code=${data.fromAcc} ORDER BY Id desc limit 1), (SELECT a3.Color FROM account a3 WHERE Code=${data.fromAcc} ORDER BY Id desc limit 1), (SELECT a4.Status FROM account a4 WHERE Code=${data.fromAcc} ORDER BY Id desc limit 1), ${data.groupId}, '${now}')`
  )
  db.runSync(
    `INSERT INTO account (Id, Code, Active, Name, Balance, IconId, Color, Status, GroupsId, UpdateDate) VALUES (${dataToAcc[0]}, ${data.toAcc}, 1, (SELECT a.name FROM account a WHERE Code=${data.toAcc} ORDER BY Id desc limit 1), ${dataToAcc[2]}, (SELECT a2.IconId FROM account a2 WHERE Code=${data.toAcc} ORDER BY Id desc limit 1), (SELECT a3.Color FROM account a3 WHERE Code=${data.toAcc} ORDER BY Id desc limit 1), (SELECT a4.Status FROM account a4 WHERE Code=${data.toAcc} ORDER BY Id desc limit 1), ${data.groupId}, '${now}')`
  )
  db.runSync(
...
export const deleteFinance = (data) => {
  ...
  const result = db.getFirstSync(`SELECT Balance FROM account WHERE Active=1 AND GroupsId=${data.groupid} AND Code = (SELECT Code FROM finance WHERE Id=${data.id})`);
